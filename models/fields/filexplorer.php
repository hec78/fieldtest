...
class JFormFieldFileXplorer extends JFormField
{
	...
	protected function getInput()
	{
		...
		$doc->addStyleSheet($component_path."/libraries/jquery.fs.boxer.min.css");
		$doc->addScript($component_path."/libraries/jquery.fs.boxer.min.js");
		...
		$createdirurl=$this->getAttribute("createdirurl",Juri::base(true)."/index.php?option=".$option."&task=".$webservicecontroller.".createdir");
		...
		{
			$script = array();
			$script[]="var jformfieldfilexplorer_supported_extensions={doc:0, docx:0, zip:0,xls:0,xlsx:0, pdf:2,png:1, gif:1,jpg:1,jpeg:1, ico:1, html:2, htm:2, php:0}";
			$script[]="var jformfieldfilexplorer_curdir={};";
			$script[]="var jformfieldfilexplorer_curdirreadonly={};";
			$script[]="var jformfieldfilexplorer_readonly={};";
			$script[]="var jformfieldfilexplorer_rootdir={};";
			$script[]="var jformfieldfilexplorer_table={};";
			$script[]="var jformfieldfilexplorer_sitedir={};";
			$script[]="var jformfieldfilexplorer_deletefiles=[];";
			...
			$script[]="    return false; }";
			$script[]="function jformfieldfilexplorer_showCreateDirectory(id) {";
			$script[]="    jQuery( \"#\"+id+\"_createDirectoryMessageBox\" ).dialog({resizable: false, height:150,width : 400,modal: true, draggable: true});";
			$script[]="    return false; }";
			...
			$script[]="    return str;";
			$script[]="}";
			$script[]="function jformfieldfilexplorer_addRow(id,path, isfolder, isSelected,fsize,mtime,ext, right,curdir) {";
			...
			$script[]="            preview=jformfieldfilexplorer_supported_extensions[ext];";
			$script[]="            icon='file-'+ext+'.png'; ";
			$script[]="        } else { ";
			$script[]="            icon='file-default.png'; ";
			$script[]="            preview=0;";
			$script[]="        }";
			$script[]="        fullpath=curdir+'/'+path;var ico='<img src=\"".$image_folder."/'+icon+'\" width=\"16\" width=\"16\" >';";
			$script[]="        if(preview) {";
			$script[]="            ico='<a onclick=\"javascript:jformfieldfilexplorer_preview(\\''+id+'\\', \\''+fullpath+'\\');return false;\" title=\"".JText::_('JFORMFIELD_FILEXPLORER_PREVIEW_TOOLTIP')."\" >'+ico+'</a>';";
			$script[]="        }";
			$script[]="        row=[ico,path,mtime,jformfieldfilexplorer_calcSize(fsize)];cls='file';";
			$script[]="    }";
			...
			$script[]="        var tbl=jformfieldfilexplorer_table[id];";
			$script[]="        var row=tbl.row('.selected');";
			$script[]="        if (row===undefined) return;";
			$script[]="        var sitedir=jformfieldfilexplorer_sitedir[id];";
			$script[]="         var selectedfile=row.data()[1];";
			$script[]="        filename=sitedir+'/'+selectedfile;";
			$script[]="    }";
			$script[]="    jQuery('#'+id+'_PreviewDialog .content').html('<img src=\\''+filename+'\\' >');";
			$script[]="    jQuery('#'+id+'_PreviewDialog').dialog({resizable: true, modal: true, draggable: true});";
			//$script[]="    jQuery.boxer(jQuery('<img src=\\''+filename+'\\' >'));";
			//$script[]="    jQuery('#'+id+'_PreviewDialog').boxer();";
			$script[]="}";
			...
			$script[]="            items= data.list;var isrootdir=(data.dir==jformfieldfilexplorer_rootdir[id]);var basedir=data.siterootdir;";
			$script[]="    	       jformfieldfilexplorer_curdirreadonly[id]=((data.right.indexOf('w') == -1) && jformfieldfilexplorer_readonly[id]);";
			$script[]="            jformfieldfilexplorer_sitedir[id]= basedir;";
			$script[]="            if(data.error=='') {";
			$script[]="                jQuery.each( items, function( i, item ) {";
			$script[]="                    if (item.path!='..' || !isrootdir) ";
			$script[]="                        jformfieldfilexplorer_addRow(id,item.path, item.type == 'dir', (item.path in selected),item.size,item.mtime,item.ext,item.right,basedir);";
			$script[]="                });";
			...
			$script[]="    }";
 			...
 			$script[]="}";
 			$script[]="function jformfieldfilexplorer_createDir(id) {";
 			$script[]="    jQuery('#'+id+'_createDirectoryMessageBox').dialog('close');";
 			$script[]="    if (jformfieldfilexplorer_readonly[id]) { return; }";
 			$script[]="    var tbl=jformfieldfilexplorer_table[id];";
 			$script[]="    var curdir=jformfieldfilexplorer_curdir[id];";
 			$script[]="    var rootdir=jformfieldfilexplorer_rootdir[id];";
 			$script[]="    var dir=rootdir;if(curdir!='') dir=dir+curdir;";
 			$script[]="    var formData = new FormData();";
 			$script[]="    formData.append('newdir',jQuery('#'+id+'_createDirectoryMessageBox_newdir').val());";
 			$script[]="    formData.append('directory', dir);";
 			$script[]="    try {";
 			$script[]="        var ajaxQuery=jQuery.ajax({";
 			$script[]="        type: 'POST',";
 			$script[]="        url: '".$createdirurl."',";
 			$script[]="        data: formData,";
 			$script[]="        dataType: 'json',";
 			$script[]="        processData: false, // Don't process the files";
 			$script[]="        contentType: false,";
		...
		$html[]="<input type=\"hidden\" id=\"".$this->id."_selected\" name=\"".$this->name."_selected\" value=\"".$this->value."\" />";
		...
		$html[]="  </div>";
		$html[]="</div>";
		$html[]="<div id=\"".$this->id."_createDirectoryMessageBox\" title=\"".JText::_('JFORMFIELD_FILEXPLORER_CREATE_DIRECTORY_TITLE')."\" class=\"jformfieldfilexplorer_msgbox\">";
		$html[]="  <div style=\"float:left\"><img src=\"".$quetionimg."\" /></div><div id=\"".$this->id."_content\" style=\"margin-left:70px;\" class=\"content\">";
		$html[]="    ".JText::_('JFORMFIELD_FILEXPLORER_CREATE_DIRECTORY_LABEL');
		$html[]="    <input id=\"".$this->id."_createDirectoryMessageBox_newdir\" type=\"text\" value=\"\" class=\"jformfieldfilexplorer_msgbox_edit\" >";
		$html[]="    <input id=\"".$this->id."_createDirectoryMessageBox_curdir\" type=\"hidden\" value=\"\"  >";
		$html[]="  </div>";
		$html[]="  <div id=\"".$this->id."_browseButtons\" class=\"buttons center\" style=\"margin-top:30px;\">";
		$html[]="    <button onclick=\"jformfieldfilexplorer_createDir('".$this->id."');return false;\"><span class=\"icon-ok\"></span>".JText::_('JFORMFIELD_FILEXPLORER_CREATEDIR')."</button>";
		$html[]="    <button onclick=\"javascript:jQuery('#".$this->id."_createDirectoryMessageBox').dialog('close');return false;\"><span class=\"icon-cancel\"></span>".JText::_('JFORMFIELD_FILEXPLORER_CANCEL')."</button>";
		$html[]="  </div>";
		$html[]="</div>";
		$html[]="<div id=\"".$this->id."_PreviewDialog\" title=\"".JText::_('JFORMFIELD_FILEXPLORER_PREVIEW_TITLE')."\" class=\"jformfieldfilexplorer_msgbox jformfieldfilexplorer_previewmsgbox\">";
		$html[]="  <div id=\"".$this->id."_PreviewDialogContent\" class=\"content\" ></div>";
		$html[]="  <div id=\"".$this->id."_browseButtons\" class=\"buttons center\">";
		$html[]="    <button onclick=\"javascript:jQuery('#".$this->id."_PreviewDialog').dialog('close');return false;\"><span class=\"icon-cancel\"></span>".JText::_('JFORMFIELD_FILEXPLORER_OK')."</button>";
		$html[]="  </div>";
		$html[]="</div>";
		...
		$script[]="jformfieldfilexplorer_sitedir['".$this->id."']='';";
