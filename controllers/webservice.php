...
class FieldTestControllerWebservice extends FieldTestController
{
	...
	function getdir()
	{
		...
		$data = array('dir'=> $relatdir, 'list' => $list, 'error'=>$error,'right'=> $rw, 'siterootdir'=> $sitedir);
	 ...
	 * POST variables:
	 * directory : current directory (to list)
	 * newdir : new directory to create in current dir
	 */
	function createdir()
	{
		$error="";
		...
		{
			$dir=JFactory::getApplication()->input->get('directory','','post');
			$newdir = JFactory::getApplication()->input->get('newdir','','post');
			$rootdir = realpath(JPATH_ROOT).DIRECTORY_SEPARATOR.$dir;
			if ($newdir!='') {
				$newdirname=$rootdir.DIRECTORY_SEPARATOR.$newdir;
				if (file_exists($newdirname)) {
					$error=$newdir. " existe déjà!";
				}
				else
				{
					if (!mkdir($newdirname)) {
						$error.="Erreur creation repertoire ".$newdir;
					}
				}
			}
			else {
				$error="Aucun répertoire";
			}
		}
		$return = ($error=='');
		$data = array( 'return'=>$return, 'error'=>$error);
